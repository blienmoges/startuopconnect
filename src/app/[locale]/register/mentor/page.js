...
export default async function MentorRegistration({ params }) {
  const { locale } = await params;
  const currentLocale = locale === "am" ? "am" : "en";

  const dict = await getDictionary(currentLocale);
  const t = dict.mentorStep1;
    ...
    <div className="min-h-screen bg-[#e6eee9] font-sans text-gray-900 flex flex-col md:flex-row">
      ...
      <div className="w-full lg:w-[60%] flex flex-col items-center py-12 px-4 sm:px-6 overflow-y-auto">
        ...
        <div className="w-full max-w-3xl flex flex-col items-center">
          ...
          <div className="w-full max-w-md mb-10 px-4">
            <div className="relative flex justify-between items-start">
              ...
              <div className="flex flex-col items-center gap-2">
                ...
                <span className="text-[10px] font-medium text-gray-400">{t.steps.professional}</span>
              </div>
          ...
          {/* Form Card */}
          <div className="w-full bg-transparent rounded-2xl p-8 sm:p-12 mb-10 border-none">

            <div className="text-center mb-10">
              <h1 className="text-3xl font-bold text-[#0f3d32] mb-3">{t.main.title}</h1>
          ...
          <div className="w-full flex flex-col md:flex-row justify-between items-center gap-4 text-[10px] font-bold text-gray-500 px-2 mt-4">
            <div className="text-[#053023] text-sm tracking-tight font-black">
              {t.footer.portalMain}<span className="font-medium">{t.footer.portalSub}</span>
            </div>
            ...
            <div className="flex flex-wrap justify-center gap-4 md:gap-8 uppercase tracking-widest">
              ...
              <Link href="#" className="hover:text-[#136150] transition">{t.footer.institutionalTerms}</Link>
              <Link href="#" className="hover:text-[#136150] transition">{t.footer.accessibility}</Link>
              <Link href="#" className="hover:text-[#136150] transition">{t.footer.contactRegistry}</Link>
            </div>

            <p>&copy; 2024 {t.footer.portalMain} {t.footer.portalSub}</p>
          </div>
