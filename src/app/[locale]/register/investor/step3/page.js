...
export default async function InvestorRegistrationStep3({ params }) {
  const { locale } = await params;
  const currentLocale = locale === "am" ? "am" : "en";

  const dict = await getDictionary(currentLocale);
  const t = dict.investorStep3;
      ...
      <div className="w-full lg:w-[60%] flex flex-col pt-10 pb-32 px-4 md:px-8 overflow-y-auto">
        <div className="w-full max-w-2xl mx-auto flex flex-col">
          <div className="w-full flex items-center gap-3 mb-6 pl-4">
            <Link href={`/${currentLocale}/register/investor/step2`} className="text-[#167b66] hover:text-[#0f5c4a] transition">
