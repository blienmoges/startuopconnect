...
export default async function StartupProjectDocuments({ params }) {
  const { locale } = await params;
  const currentLocale = locale === "am" ? "am" : "en";

  const dict = await getDictionary(currentLocale);
  const t = dict.startupProjectDocuments;
      ...
      <main className="flex-grow flex flex-col overflow-y-auto">
        ...
        <div className="px-4 sm:px-10 py-10 w-full max-w-[1000px] mx-auto">
          ...
          <div className="flex justify-center mb-10">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-[#0f3d32]">{t.stepper.project}<br />{t.stepper.info}</span>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-[#0f3d32]">{t.stepper.documents}</span>
              </div>
          ...
          <div className="bg-[#fff1f1] rounded-xl p-5 mb-8 flex items-start gap-4 border border-[#ffd1d1]">
            ...
            <div>
              <h4 className="text-sm font-bold text-red-600 mb-0.5">{t.error.title}</h4>
              <p className="text-xs text-red-500 font-medium">{t.error.description}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-10">
            ...
            <div className="bg-white rounded-xl border-2 border-[#0f3d32] p-6 flex flex-col items-center justify-center text-center shadow-sm">
              ...
              <h4 className="text-sm font-bold text-[#0f3d32] mb-1">{t.cards.pitchDeck.title}</h4>
              <p className="text-[10px] text-gray-500 font-medium mb-4">{t.cards.pitchDeck.requirement}</p>
            ...
            <div className="bg-[#f8fafc] rounded-xl border border-gray-200 p-6 flex flex-col items-center justify-center text-center">
              ...
              <h4 className="text-sm font-bold text-[#0f3d32] mb-1">{t.cards.businessPlan.title}</h4>
              <p className="text-[10px] text-gray-500 font-medium mb-4">{t.cards.businessPlan.requirement}</p>

              <span className="text-[11px] font-bold text-gray-400 mb-6">{t.status.missing}</span>
            ...
            <div className="bg-[#f8fafc] rounded-xl border border-gray-200 p-6 flex flex-col items-center justify-center text-center">
              ...
              <h4 className="text-sm font-bold text-[#0f3d32] mb-1">{t.cards.financialDoc.title}</h4>
              <p className="text-[10px] text-gray-500 font-medium mb-4">{t.cards.financialDoc.requirement}</p>
            ...
            <div className="bg-white rounded-xl border-2 border-red-600 p-6 flex flex-col items-center justify-center text-center shadow-[0_4px_14px_rgba(220,38,38,0.1)] relative">
              ...
              <h4 className="text-sm font-bold text-[#0f3d32] mb-1">{t.cards.demoVideo.title}</h4>
              <p className="text-[10px] text-gray-500 font-medium mb-3">{t.cards.demoVideo.requirement}</p>

              <div className="bg-red-700 text-white px-3 py-1 rounded-full text-[10px] font-bold mb-2">
                {t.status.invalidFormat}
              </div>
              <p className="text-[9px] text-red-500 font-medium mb-5">{t.cards.demoVideo.error}</p>
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden mb-12 shadow-sm">
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-[#fcfcfd]">
              <h3 className="text-sm font-bold text-[#0f3d32]">{t.table.title}</h3>
              <span className="text-[11px] font-bold text-gray-400">{t.table.progress}</span>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-gray-100">
                    <th className="px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest bg-gray-50/50">{t.table.documentName}</th>
                    <th className="px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest bg-gray-50/50">{t.table.type}</th>
                    <th className="px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest bg-gray-50/50">{t.table.uploadDate}</th>
                    <th className="px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest bg-gray-50/50">{t.table.status}</th>
                    <th className="px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest bg-gray-50/50 text-right">{t.table.action}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  <tr className="hover:bg-gray-50/50 transition">
                    ...
                    <td className="px-6 py-5">
                      <span className="text-xs text-gray-600">{t.table.uploadDateValue}</span>
                    </td>
                    <td className="px-6 py-5">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 pt-6 border-t border-gray-200">
            <Link href={`/${currentLocale}/startup/project`} className="px-8 py-3.5 bg-white border border-[#0f3d32] text-[#0f3d32] font-bold rounded-lg hover:bg-gray-50 transition text-sm shadow-sm">
              {t.actions.backToProjectInfo}
            </Link>
            <div className="flex items-center gap-4 ml-auto">
              <span className="text-[11px] text-gray-400 font-medium italic hidden sm:block">{t.actions.autoSaving}</span>
