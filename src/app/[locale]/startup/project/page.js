...
export default async function StartupProjectsListing({ params }) {
  const { locale } = await params;
  const currentLocale = locale === "am" ? "am" : "en";

  const dict = await getDictionary(currentLocale);
  const t = dict.startupProjectsListing;
      ...
      <main className="flex-grow flex flex-col overflow-y-auto relative">
        ...
        <div className="px-4 sm:px-10 py-10 w-full max-w-[1200px] mx-auto pb-24">
          ...
          <div className="flex justify-between items-start mb-8">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2 tracking-tight">{t.header.title}</h1>
              <p className="text-sm text-gray-500 font-medium">{t.header.subtitle}</p>
            </div>
            <Link href={`/${currentLocale}/startup/project/create`} className="px-6 py-3 bg-[#0f3d32] hover:bg-[#0a2921] text-white font-bold rounded-xl transition shadow-md text-sm flex items-center gap-2">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 4v16m8-8H4"></path></svg>
              {t.header.createNewProject}
            </Link>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-5 mb-10">
            ...
            <div className="bg-white rounded-[20px] p-6 shadow-sm border border-gray-100 flex items-center justify-between">
              <div>
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">{t.stats.totalProjects}</p>
            ...
            <div className="bg-white rounded-[20px] p-6 shadow-sm border border-gray-100 flex items-center justify-between">
              <div>
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">{t.stats.published}</p>
            ...
            <div className="bg-white rounded-[20px] p-6 shadow-sm border border-gray-100 flex items-center justify-between">
              <div>
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">{t.stats.drafts}</p>
            ...
            <div className="bg-white rounded-[20px] p-6 shadow-sm border border-gray-100 flex items-center justify-between">
              <div>
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">{t.stats.applications}</p>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div className="flex flex-wrap items-center gap-2 sm:gap-6">
              <button className="px-5 py-2 bg-[#0f3d32] text-white text-xs font-bold rounded-full transition shadow-sm">{t.filters.all}</button>
              <button className="px-2 py-2 text-gray-600 hover:text-gray-900 text-xs font-bold transition">{t.filters.draft}</button>
              <button className="px-2 py-2 text-gray-600 hover:text-gray-900 text-xs font-bold transition">{t.filters.published}</button>
              <button className="px-2 py-2 text-gray-600 hover:text-gray-900 text-xs font-bold transition">{t.filters.underReview}</button>
              <button className="px-2 py-2 text-gray-600 hover:text-gray-900 text-xs font-bold transition">{t.filters.funded}</button>
            </div>
            ...
            <div className="relative w-48">
              <select className="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#0f3d32]/20 transition text-xs font-bold text-gray-700 appearance-none shadow-sm">
                ...
                <option>{t.filters.sortOldest}</option>
                <option>{t.filters.sortFundingGoal}</option>
              </select>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row overflow-hidden relative border-l-4 border-l-gray-300">
              ...
              <div className="flex-grow p-6 flex flex-col">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span className="bg-gray-100 text-gray-600 text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-md">{t.status.draft}</span>
                </div>

                <div className="flex flex-wrap items-center gap-2 mb-6">
                  <span className="bg-[#f0f2f5] text-gray-600 text-[9px] font-bold uppercase tracking-widest px-2 py-1 rounded">{t.tags.agricultureTechnology}</span>
                  <span className="bg-[#fff7ed] text-[#ea580c] text-[9px] font-bold uppercase tracking-widest px-2 py-1 rounded">{t.tags.prototype}</span>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8 border-t border-gray-100 pt-6">
                  <div>
                    <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">{t.details.fundingGoal}</p>
                    <p className="text-sm font-bold text-gray-900">$50,000</p>
                  </div>
                  <div>
                    <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">{t.details.progress}</p>
                  ...
                  <div>
                    <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">{t.details.lastUpdated}</p>
                    <p className="text-sm text-gray-600">{t.cards.agriTech.lastUpdated}</p>
                  </div>
                  <div>
                    <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">{t.details.inquiries}</p>
                    <p className="text-sm text-gray-600">{t.cards.agriTech.inquiries}</p>
                  </div>
                </div>

                {/* Actions */}
                <div className="flex items-center justify-end gap-6 mt-auto">
                  <Link href={`/${currentLocale}/startup/project/create`} className="text-xs font-bold text-gray-600 hover:text-gray-900 transition flex items-center gap-1.5">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                    {t.actions.editProject}
                  </Link>
                  <Link href={`/${currentLocale}/startup/project/documents`} className="px-6 py-2.5 bg-[#0f3d32] hover:bg-[#0a2921] text-white font-bold rounded-lg transition shadow-sm text-xs">
                    {t.actions.continueDocuments}
                  </Link>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row overflow-hidden relative border-l-4 border-l-[#0f3d32]">
              ...
              <div className="flex-grow p-6 flex flex-col">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span className="text-[#10b981] text-[10px] font-bold uppercase tracking-widest">{t.status.published}</span>
                </div>

                <div className="flex flex-wrap items-center gap-2 mb-6">
                  <span className="bg-[#f0f2f5] text-gray-600 text-[9px] font-bold uppercase tracking-widest px-2 py-1 rounded">{t.tags.renewableEnergy}</span>
                  <span className="bg-[#eff6ff] text-[#2563eb] text-[9px] font-bold uppercase tracking-widest px-2 py-1 rounded">{t.tags.mvp}</span>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8 border-t border-gray-100 pt-6">
                  ...
                  <div>
                    <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">{t.details.lastUpdated}</p>
                    <p className="text-sm text-gray-600">{t.cards.greenEnergy.lastUpdated}</p>
                  </div>
                  <div>
                    <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">{t.details.inquiries}</p>
                    <p className="text-sm text-gray-600">{t.cards.greenEnergy.inquiries}</p>
                  </div>
                </div>

                {/* Actions */}
                <div className="flex items-center justify-end gap-6 mt-auto">
                  <button className="text-xs font-bold text-gray-600 hover:text-gray-900 transition">{t.actions.viewDetails}</button>
                  <button className="text-xs font-bold text-gray-600 hover:text-gray-900 transition">{t.actions.updateProgress}</button>
                  <button className="px-6 py-2.5 bg-[#0f3d32] hover:bg-[#0a2921] text-white font-bold rounded-lg transition shadow-sm text-xs">
                    {t.actions.applyForInvestment}
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row overflow-hidden relative border-l-4 border-l-[#f59e0b]">
              ...
              <div className="flex-grow p-6 flex flex-col">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span className="text-[#ea580c] text-[10px] font-bold uppercase tracking-widest">{t.status.underReview}</span>
                </div>

                <div className="flex flex-wrap items-center gap-2 mb-6">
                  <span className="bg-[#f0f2f5] text-gray-600 text-[9px] font-bold uppercase tracking-widest px-2 py-1 rounded">{t.tags.edtech}</span>
                  <span className="bg-[#f5f3ff] text-[#7c3aed] text-[9px] font-bold uppercase tracking-widest px-2 py-1 rounded">{t.tags.earlyStage}</span>
                  <div className="ml-auto bg-[#f0fdf4] text-[#16a34a] border border-[#bbf7d0] px-3 py-1 rounded-full text-[10px] font-bold flex items-center gap-1.5">
                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7"></path></svg>
                    {t.status.complete}
                  </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8 border-t border-gray-100 pt-6">
                  ...
                  <div>
                    <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">{t.details.lastUpdated}</p>
                    <p className="text-sm text-gray-600">{t.cards.eduSmart.lastUpdated}</p>
                  </div>
                  <div>
                    <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">{t.details.inquiries}</p>
                    <p className="text-sm text-gray-600">{t.cards.eduSmart.inquiries}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-end gap-6 mt-auto">
                  <button className="text-xs font-bold text-gray-600 hover:text-gray-900 transition">{t.actions.viewDetails}</button>
                  <button className="text-xs font-bold text-gray-600 hover:text-gray-900 transition">{t.actions.editProject}</button>
                  <button disabled className="px-6 py-2.5 bg-[#f1f5f9] text-gray-400 font-bold rounded-lg text-xs cursor-not-allowed">
                    {t.actions.reviewInProgress}
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className="w-full border border-dashed border-gray-200 bg-[#f8fafc] rounded-3xl p-10 flex flex-col items-center justify-center text-center">
            ...
            <h3 className="text-xl font-bold text-gray-900 mb-2 tracking-tight">{t.empty.title}</h3>
            <p className="text-sm text-gray-500 max-w-sm mb-8 leading-relaxed">
              {t.empty.description}
            </p>
            <Link href={`/${currentLocale}/startup/project/create`} className="px-8 py-3.5 bg-[#0f3d32] hover:bg-[#0a2921] text-white font-bold rounded-xl transition shadow-md text-sm">
